...
def optKeyFrac(p, m, QBER, s , f ):
    beta,xi, nu = p
    return 1-keyFraction(m, QBER, beta, xi, nu, s, f)

def keyFraction(m,QBER,beta,xi,nu, s ,f ): # m is the initial raw raw key length
...
def minimize(n, qber, s , f , loops = 1, steps =101):
...
fp = open("result_ourcascade.csv","w")
fp.write("m,l\n")
for m,qber,f in zip(N,QBER,fvals):
    print (m,qber,f)
    l, par = minimize(m, qber, s=s, f =f)

    fp.write(str(m)+","+str(l)+"\n")
fp.close()
