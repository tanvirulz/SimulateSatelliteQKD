...
def ChannelLoss(Distance, WidthAtReceiver, ApertReceive, Elevation, AtmosAtten, PointingOffset = 0):
    ...
    if Distance.size > 1:
        ...
        for i in range(len(Distance)):
            ...
            Lgeom = np.exp(-2*(Off[i]**2)/(w_d[i]**2))*mpmath.nsum(lambda k:((2**k)*Off[i] **(2*k))/(w_d[i]**(2*k)*math.gamma(k + 1)**2),[0, np.inf])
        ...
        #Lgeom = np.exp(-2*(Off**2)/(w_d**2))*mpmath.nsum(lambda k:((2**k)*Off **(2*k))/(w_d**(2*k)*math.factorial(k)**2)*special.gammainc((float(k)+1), 2*(r**2)/(w_d**2)),[0, np.inf])
        #Lgeom = 10*np.log10(float(Lgeom)) 
        Lgeom = 1 # test
